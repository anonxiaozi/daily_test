...
class SplitDB(object):
    ...
    def process_increment(self):
        incre_date = self.args["increment"]
        if not incre_date:
            return
        try:
            str_data = incre_date.split("-")
            if len(str_data) == 2:
                s, e = datetime.datetime.strptime(str_data[0], "%Y%m%d"), datetime.datetime.strptime(str_data[1], "%Y%m%d")
                match = {
                    'date': {
                        '$gte': s,
                        '$lt': e
                ...
                self.filter_rule.insert(0, match)
            elif len(str_data) == 1:
                s = datetime.datetime.strptime(str_data[0], "%Y%m%d")
                match = {
                    'date': {
                        '$gte': s
                    }
                ...
                print("Increment Date Error: {}".format(incre_date))
    ...
    def storeid_unique(self):
        new_data = self.get_data()
        cursor = self.write_table.find()
        db_data = [x for x in cursor]
        new_dict = {x["storeId"]: x for x in new_data}
        db_dict = {x["storeId"]: x for x in db_data}
        new_keys = set(new_dict.keys())
        db_keys = set(db_dict.keys())
        both_keys = list(new_keys.intersection(db_keys))
        if both_keys:
            for item in both_keys:
                new_dict.pop(item)
        new_data = list(new_dict.values())
        return new_data
...
def get_args():
    ...
    arg.add_argument("--increment", type=str, help="increment date, example: '20190104-20190304'")
    return arg
